...
export default function Calender({
  ...
  const openEditModal = (day) => {
    const moodData = completeData?.[selectedYear]?.[numericMonth]?.[day] || {}; 
    setSelectedDate(`${selectedYear}-${String(numericMonth + 1).padStart(2, '0')}-${String(day).padStart(2, '0')}`);
    console.log("existing data:", moodData);
    setExistingData(moodData);
    setIsModalOpen(true);
  };
  return (
    <div className="flex flex-col gap-4">
      <div className="grid grid-cols-3 gap-4">
        ...
        >
          <FaChevronCircleLeft className="text-indigo-400 text-lg sm:text-xl duration-200 hover:opacity-60" />
        </button>
        <p
          className={"text-center capitalized textGradient " + fugaz.className}
        >
        ...
        >
          <FaChevronCircleRight className="text-indigo-400 text-lg sm:text-xl duration-200 hover:opacity-60" />
        </button>
      </div>
          ...
          return (
            <div key={rowIndex} className="grid grid-cols-7 gap-1">
              {dayList.map((_, dayOfWeekIndex) => {
                ...
                : data[String(dayIndex)]?.mood
                ? gradients.indigo[data[String(dayIndex)].mood]
                : "white";
                return (
                  <div
                  onClick={() => openEditModal(dayIndex)}
      ...
      {isLogin && <Modal isOpen={isModalOpen} setIsOpen={setIsModalOpen} selectedDate={selectedDate} existingData={existingData} onSave={handleSetMood}/>}
    </div>
  );
}
